...
temperature_data=pd.read_csv(sonic_files[file_num], skiprows=1)
...
print(w_theta[2])
average_in=60
heat_flux=calculate_heat_flux(w_theta,column_data_uz,potential_temp2,average_in)
...
plt.xlabel('time')
plt.ylabel("w'theta'")
plt.title('time average of '+str(average_in)+" min")
# plt.grid(True)
# plt.legend()
plt.show()

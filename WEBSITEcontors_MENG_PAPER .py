...
fig, ax = plt.subplots(nrows=1, ncols=2,subplot_kw={'projection': crs.PlateCarree()},figsize=(2.9, 10.9),dpi=500)
...
dirs=['clz_0p01_second_file_katrina_10_to_20','clz_1p0_second_file_katrina',] #,'clz_0p01','clz_100']
dir_num=0
...
for dir in dirs:
    ...
    ax[i+1].contourf(to_np(lons1),to_np(lats1), (total_wind), 250,  vmin=wspd_min,vmax=wspd_max,     transform=crs.PlateCarree(), 
        cmap=my_cmap2,)
    ax[i+1].background_patch.set_facecolor('black')    
    ax[i].background_patch.set_facecolor('black')  
...
###for log##
cbar1=plt.colorbar(contor, ax=ax[0],orientation='vertical', fraction=0.04
,extend='min')
print('Im here',cbar1.ax.get_yticklabels())
cbar1.ax.tick_params(labelsize=4)
cbar1.set_label(label=r'$z _{ 0 }$  $\mathrm{ (\,m) \,} $',size=4 ,)
# cbar1=plt.colorbar(contor, ax=ax[0],orientation='vertical', fraction=0.1
# ,extend='min',)
# cbar1.ax.tick_params(labelsize=8)
# cbar1.set_label(label=r'$z_{ 0 }$  $\mathrm{ (\,m) \,}$',size=8,)
####
####for linear####
...
ax=ax[1], orientation='vertical',  extend='max', fraction=0.04,

)
cbar2.ax.tick_params(labelsize=4)
# r'$\mathrm{(\,ms^{-1}) \,}$'
cbar2.set_label(label=r"Surface wind speed $\mathrm{(m \:s^{-1}) \,}$",size=4)
# cbar2=fig.colorbar(mpl.cm.ScalarMappable(norm=norm2, cmap=my_cmap2),
...
ax[0].annotate('a)', xy=(0.1, 0.99), xycoords='axes fraction',
             xytext=(0, 0), textcoords='offset points',
             ha="right", va="top",size=4,
             color='white'
             )
ax[1].annotate('b)', xy=(0.1, 0.99), xycoords='axes fraction',
             xytext=(0, 0), textcoords='offset points',
             ha="right", va="top",size=4,
             color='white'
             )
...
# ax[0].legend(h,l)
plt.savefig('/Users/lmatak/Desktop/building_website/Images/CLZ_katrina_dpi100.png',bbox_inches='tight')
